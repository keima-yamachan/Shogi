...
function 駒描画(駒, x, y, 指し手灰表示, 反転){
    ...
    var piece = Snap(".shogiban-okiba").g();
    var 文字 = 駒;
    if (駒.match('_')) {
        文字 = 駒.replace('_', '');
    }
...
function 将棋盤描画(先手名, 後手名, 評価値, 反転, 指し手) {
    ...
    var board = Snap(".shogiban-okiba").g();
    //外枠色のみ
...
function viewControl() {
    $(".shogiboard").append('<ul class="control-buttons">');
    $('ul.control-buttons').append('<li class="btn1 " onclick="first()">最初へ</li>');
    $('ul.control-buttons').append('<li class="btn2" onclick="prev()">戻る</li>');
    $('ul.control-buttons').append('<li class="btn3" onclick="next()">次へ</li>');
    $('ul.control-buttons').append('<li class="btn4" onclick="last()">最後へ</li>');
    $('ul.control-buttons').append('<li class="btn5" onclick="reverse()"><span>反転</span></li>');
    $('ul.control-buttons').wrap('<div class="control-area" />');
}

var isDo = false;
$(document).on('touchend', '.shogiban-okiba', function(event) {
    if (isDo) {
        ...
        var ele = document.querySelector('.shogiban-okiba');
        var eleBan = document.querySelector('.shogiboard-space');
...
//盤クリック関数
$(document).on('click', '.shogiban-okiba', function (event) {
